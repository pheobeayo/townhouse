...
export default function Nav(props: NavProp) {
  ...
  return (
    <div className="flex w-full items-center justify-between flex-wrap md:flex-nowrap gap-3">
      ...
      <button onClick={() => openDialog("new_features_dialog")}>
        <IoIosNotificationsOutline className="w-[24px] h-[24px] hidden lg:block" />
      </button>
      <form className="flex gap-2 text-[gray-7-text] items-center justify-center border-[1px] px-4 py-2 rounded-[20px] border-[var(--gray-5-stroke)] order-last md:-order-none w-full md:w-auto">
        <CiSearch className="w-[20px] h-[20px]" />
        <input
          type="text"
          placeholder="Search"
          className="outline-none sm:w-[400px]"
        />
      </form>
      <div className="flex justify-center items-center gap-2">
        <CiLocationOn className="w-[16px] h-[16px] hidden lg:block" />
