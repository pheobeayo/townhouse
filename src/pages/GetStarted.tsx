...
import background from "../assets/images/backgrounds/Screen2.jpg";
...
function GetStarted() {
  ...
  window.onresize = () => {
    ...
    if (screen_width > 768) {
      setScreenBackground(
        `linear-gradient(0deg, rgba(0, 0, 0, 0.5), rgba(0, 0, 0, 0.5)),url(${background}),no-repeat`
      );
      setShowLogo(true);
    } else {
  ...
  return (
    <main
      ...
      className={`flex h-screen justify-center max-md:flex-col max-md:items-center `}
    >
      {showLogo ? (
        <Link
          to="/"
          className="mr-auto h-full flex-grow flex items-center justify-center"
        >
      ...
      <div className="md:ml-auto sm:m-[40px] items-center sm:rounded-sm sm:shadow-lg bg-white flex flex-col sm:w-[520px] max-sm:px-[3vw]">
        <div className="sm:w-[360px] my-[40px]">
          ...
          <div className="flex flex-col gap-[12px] text-sm">
            <button
              onClick={() =>
                (window.location.href = `${API_URL}/auth/googleOAuth/google`)
              }
              className={`flex justify-center items-center py-3 px-6  bg-white border-[1px] rounded-lg`}
            >
              <img
                src={googleIcon}
                alt="google's icon"
                className="sm:w-[24px] mr-auto sm:h-[24px]"
              />
            ...
            <button
              onClick={() => navigate("/sign_up_with_email")}
              className={`flex justify-center items-center py-3 px-6  bg-white border-[1px] rounded-lg`}
            >
              <img
                src={emailIcon}
