...
import sample9 from "../assets/images/sample9.svg";
import sample10 from "../assets/images/sample10.svg";
...
export default function Home() {
  ...
  const bulletins: Bulletin[] = [
    {
      title: "Latest Update",
      description: `The street light in front of Harvest Haven market has been broken for weeks.When is it going to get fixed? `,
            ...
            {events.length > 0 ? (
              <div className="grid max-sm:grid-cols-1 gap-2 gap-y-4 sm:grid-cols-1 md:grid-cols-2">
                {events.slice(0, 2).map((event) => {
                  return (
                    <div
                      className="flex rounded-[20px] gap-4 border-[1px] flex-col p-3"
                      key={event.id}
                    >
                      <div className="flex gap-4">
                        <button
                          onClick={() => openDialog(`image_dialog_${event.id}`)}
                          className="flex flex-col items-center justify-center rounded-md bg-gray-100"
                        >
                          <img
                            src={`${API_URL}/drive/download/${event.event_photo}`}
                            alt=""
                            className="max-md: w-full md:w-[250px] h-[136px] object-cover"
                          />
                        </button>
                      ...
                      <div className="flex justify-between items-center">
                        ...
                        <div className="flex gap-3 items-center justify-center">
                          <button
                            ...
                            className="flex items-center h-[36px] bg-[var(--secondary-07)] px-3 rounded-md justify-center"
                          >
                            View Details
                          </button>
                      ...
                      <ImageDialog
                        data={{
                          ...
                          imageUrl: `${API_URL}/drive/download/${event.event_photo}`,
                          id: `${event.id}`,
                        }}
                      />
                    </div>
              ...
              <div className="flex-grow h-[40vh] gap-1 flex flex-col items-center justify-center">
                ...
                <Link
                  to="/create_event"
                  className="text-[var(--primary-01)] text-base"
                >
                  Create an event
                </Link>
              </div>
